...
class InstructionMemoryManagementUnit extends Module {
  ...
  switch(state) {
    is(tlb_S) {
      ...
      when(acked) {
        error := checkPagePermissionLevel2(
          leafPTE,
          privilege,
          true.B,
          false.B,
    ...
    is(pt1_S) {
      when(tlb.io.result.hit) {
        ...
        io.paddr.bits := Cat(
          tlb.io.result.pte.ppn1,
          tlb.io.result.pte.ppn0,
          last_vaddr.offset
        )(31, 0)
    ...
    is(pt2_S) {
      io.error := checkPagePermissionLevel1(
        midPTE,
        privilege,
        true.B,
        false.B,
      ...
      when(io.error.en) {
        ...
        io.bus.addr := Cat(midPTE.ppn1, last_vaddr.vpn0, last_vaddr.offset)(
          31,
          0
        )
        io.bus.stb := true.B
